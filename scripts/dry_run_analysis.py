    ...
    for root, dirs, filenames in os.walk(vault_path):
        ...
        for filename in filenames:
            if (
                ...
                and not filename.startswith(("📍", "MOC"))
                and "MOC" not in filename
            ):
...
def generate_dry_run_report(results: Dict[str, Any]) -> str:
    ...
    # Sort MOCs by count
    sorted_mocs = sorted(
        results["moc_distribution"].items(), key=lambda x: x[1], reverse=True
    )
...
<body>
    ...
    for file_info in results["file_analysis"]:
        confidence_class = (
            "confidence-high"
            if file_info["confidence"] > 0.7
            else "confidence-medium"
            if file_info["confidence"] > 0.4
            else "confidence-low"
        )
        html += f"""
...
def main():
    ...
    print(
        f"📊 Analyzed {results['sample_size']} files out of {results['total_files']} total"
    )
    print(f"📈 MOC Distribution:")
    for moc, count in sorted(
        results["moc_distribution"].items(), key=lambda x: x[1], reverse=True
    ):
        percentage = (
            (count / sum(results["moc_distribution"].values()) * 100)
            if sum(results["moc_distribution"].values()) > 0
            else 0
        )

...
def test_main_initializes_processing_context(monkeypatch):
    ...
    linker.main(enable_dashboard=False, dashboard_update_interval=1, testing_mode=True)
...
def test_main_falls_back_to_default_context_on_failure(monkeypatch):
    """If context creation fails, the default context should be used."""

    import obsidian_auto_linker_enhanced as linker

    # Force context creation to fail
    monkeypatch.setattr(
        linker, "create_processing_context", lambda *args, **kwargs: (_ for _ in ()).throw(RuntimeError("boom"))
    )
    ...
    # Avoid side effects after provider checks
    monkeypatch.setattr(linker, "call_ollama", fake_call_ollama)
    monkeypatch.setattr(linker, "call_ai_provider", fake_call_ai_provider)
    monkeypatch.setattr(linker, "load_progress", lambda: None)
    monkeypatch.setattr(linker, "load_cache", lambda: None)
    ...
    assert captured["ollama_context"] is DEFAULT_PROCESSING_CONTEXT
    assert captured["ai_context"] is DEFAULT_PROCESSING_CONTEXT
